 ...
 */
public class JiMiJcCastTicketThread implements Runnable {
    ...
    public JiMiJcCastTicketThread(List<CodeInfo> tickets, VoteVo vote, TicketService ticketService, Logger logger, CountDownLatch countDownLatch) {
    ...
    public void run() {
        try {
            ...
            logger.info("[吉米竞彩提票]-> 请求报文体如下\n" + strBody);
            JXmlWapper xml = JiMiTicketUtil.parseUrl(JiMiTicketUtil.Cast_Order, new String[]{JiMiTicketUtil.getLotteryTypeMap(vote.getPlayType()), "", strBody}, vote);
            if(StringUtil.isEmpty(xml.toXmlString())) {
                logger.error("[吉米竞彩提票]-> 返回响应为空");
            }
            logger.info("[吉米竞彩提票]-> 返回响应体如下\n" + xml.toXmlString("UTF-8"));
            ...
            for(int x = 0; x < count; x++) {
                ...
                if("0".equals(code)) {
                    ...
                    if(row > 0) {
                        logger.info("[吉米竞彩提票]-> 提票成功 网站票号=" + orderId);
                    } else {
                        logger.error("[吉米竞彩提票]-> 提票失败 [更新数据库失败] 网站票号=" + orderId);
                    }
                } else {
                    ...
                    logger.error("[吉米竞彩提票]-> 提票失败 " + desc + " 网站票号=" + orderId);
                }
            }
        } catch (Exception e) {
