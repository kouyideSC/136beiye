 ...
 */
public class NuoMiJcCastTicketThread implements Runnable {
    ...
    public NuoMiJcCastTicketThread(List<CodeInfo> tickets, VoteVo vote, TicketService ticketService, Logger logger, CountDownLatch countDownLatch) {
        ...
        try {
            ...
            for(int x = 0; x < count; x++) {
                ...
                if("0".equals(code)) {
                    ...
                    if(row > 0) {
                        logger.info("[糯米竞彩提票]-> 提票成功 网站票号=" + orderId);
                    } else {
                        logger.error("[糯米竞彩提票]-> 提票失败 [更新数据库失败] 网站票号=" + orderId);
                    }
                } else {
                    ...
                    logger.error("[糯米竞彩提票]-> 提票失败 " + desc + " 网站票号=" + orderId);
                }
            }
        } catch (Exception e) {
