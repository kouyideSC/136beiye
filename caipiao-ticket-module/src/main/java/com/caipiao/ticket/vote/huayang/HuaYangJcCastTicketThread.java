 ...
 */
public class HuaYangJcCastTicketThread implements Runnable {
    ...
    public HuaYangJcCastTicketThread(List<CodeInfo> tickets, VoteVo vote, TicketService ticketService, Logger logger, CountDownLatch countDownLatch) {
    ...
    public void run() {
        try {
            ...
            for (CodeInfo cb : tickets) {
                ...
                body.append("</lotteryid><issue>20000</issue><childtype>");
                body.append(cb.getPass());
                ...
                body.append("</lotteryvalue><betlotterymode>");
                body.append(cb.getCodeCopy());
                body.append("</betlotterymode></element>");
            }
            String request = HuaYangTicketUtil.getRequestContent(HuaYangTicketUtil.Cast_Jc_Order, body.toString(), vote);
            logger.info("[华阳竞彩提票]-> 请求报文体如下\n" + request);
            JXmlWapper xml = JXmlWapper.parseUrl(vote.getApiUrl(), request, "UTF-8", 30);
            if(StringUtil.isEmpty(xml.toXmlString())) {
                logger.error("[华阳竞彩提票]-> 返回响应为空");
            }
            logger.info("[华阳竞彩提票]-> 返回响应体如下\n" + xml.toXmlString("UTF-8"));
            ...
            if(resultCode.equals("0")) {
                ...
                for(int x = 0; x < count; x++) {
                    ...
                    if("0".equals(code)) {
                        ...
                        if(row > 0) {
                            logger.info("[华阳竞彩提票]-> 提票成功 网站票号=" + orderId);
                        } else {
                            logger.error("[华阳竞彩提票]-> 提票失败 [更新数据库失败] 网站票号=" + orderId);
                        }
                    } else {
                        ...
                        logger.error("[华阳竞彩提票]-> 提票失败 " + desc + " 网站票号=" + orderId);
                    }
                }
            }
        } catch (Exception e) {
