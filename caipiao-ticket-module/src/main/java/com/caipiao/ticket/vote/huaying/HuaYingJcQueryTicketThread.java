 ...
 */
public class HuaYingJcQueryTicketThread implements Runnable {
    ...
    public HuaYingJcQueryTicketThread(List<SchemeTicket> tickets, VoteVo vote, TicketService ticketService, Logger logger, CountDownLatch countDownLatch) {
                ...
                if("0".equals(code)) {
                    ...
                    for(int c = 0; c < ticketCount; c++) {
                        ...
                        if(StringUtil.isNotEmpty(codeSp)) {
                            ...
                            if (row > 0) {
                                logger.info("[华盈竞彩出票查询]-> 出票成功 网站票号=" + orderId);
                            } else {
                                logger.error("[华盈竞彩出票查询]-> 出票失败 [更新数据库失败] 网站票号=" + orderId);
                            }
                        }
                    }
                }
                else if("1".equals(code)) {
                    logger.info("[华盈竞彩出票查询]-> 出票中 网站票号=" + orderId);
                }
                else {
                    ...
                    logger.error("[华盈竞彩出票查询]-> 出票失败 " + desc + " 网站票号=" + orderId);
                }
            }
        } catch (Exception e) {
    ...
    public String formatCodeSp(String codeSp, SchemeTicket ticket) throws Exception {
        ...
        logger.info("[华盈竞彩出票查询]-> 格式化出票商返回出票SP=" + codeSp + " 票号=" + ticket.getTicketId());
        ...
        if(ticket.getPlayTypeId().equals(LotteryConstants.JCLQRFSF) || ticket.getPlayTypeId().equals(LotteryConstants.JCLQDXF)) {//让分胜负|大小分
            ...
            for(Map.Entry<String, String> voteLotterys : HuaYingTicketUtil.voteLotteryPlayType.entrySet()) {
