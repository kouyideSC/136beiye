 ...
 */
public class HuaYingAwardTicketThread implements Runnable {
    ...
    public HuaYingAwardTicketThread(List<SchemeTicket> tickets, VoteVo vote, TicketService ticketService, Logger logger, CountDownLatch countDownLatch) {
        ...
        try {
            ...
            for (int x = 0; x < count; x++) {
                ...
                if (("3".equals(code) || "4".equals(code)) && prize > 0) {
                    ...
                    if (row > 0) {
                        logger.info("[华盈兑奖]-> 兑奖成功 出票商返回中奖金额=" + prize + " 网站票号=" + orderId);
                    } else {
                        logger.error("[华盈兑奖]-> 兑奖失败 [更新数据库失败] 网站票号=" + orderId);
                    }
                } else if ("2".equals(code)) {
                    logger.info("[华盈兑奖]-> 兑奖成功 未中奖 网站票号=" + orderId);
                    ...
                    logger.info("[华盈兑奖]-> 未开奖 网站票号=" + orderId);
                } else {
                    ...
                    logger.error("[华盈兑奖]-> 兑奖失败 " + desc + " 网站票号=" + orderId);
                }
            }
        } catch (Exception e) {
            logger.error("[华盈兑奖]-> 出票查询异常", e);
        } finally {
