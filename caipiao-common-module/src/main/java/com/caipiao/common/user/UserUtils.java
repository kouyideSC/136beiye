package com.caipiao.common.user;

import com.caipiao.common.keyword.SensitiveWordUtil;
...
public class UserUtils {
     ...
     * @return
     */
    public static boolean checkNickName(String nickName, List<SensitiveWord> sensitiveWordList) throws Exception{
        if (StringUtils.isBlank(nickName)) {
            return false;
        }
        //关键词库检查
        if(StringUtil.isNotEmpty(sensitiveWordList) && SensitiveWordUtil.isSensitiveWord(nickName, sensitiveWordList)) {
            return false;
        }
        // 字符长度3-16
        int len = nickName.getBytes("GBK").length;
        if (len < 4 || len > 16) {
            return false;
        }
        // 只能由汉字字母数字下划线组成
