 ...
 * @author  mcdog
 */
public class Lottery1010Utils extends LotteryUtils
{
    /**
     * 生成期次(一年期次)
     * @author  mcdog
    ...
    public List<Map<String,String>> createPeriodsOfYear(String lotteryCloseTime, Calendar calendar)
    {
        ...
        for(int i = 0; i < 7; i ++)
        {
            if(lastYearLastKjCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.TUESDAY
                    || lastYearLastKjCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.THURSDAY
                    || lastYearLastKjCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY)
            {
                break;
            }
        ...
        //获取本年第一期次开奖的详细时间
        Calendar firstKjCalendar = getLotteryFirstKjDateOfYear(LotteryConstants.SSQ,calendar);
        ...
        for(int i = 0; i < dayOffset; i ++)
        {
                    ...
                    || firstKjCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY)
            {
                ...
                if(!isCloseTime)
                {
                    ...
                    periodMap.put("lotteryId",LotteryConstants.SSQ);
                    ...
                    periodMap.put("sellEndTime",DateUtil.formatDate(getLotterySellEndTime(
                            LotteryConstants.SSQ,firstKjCalendar).getTime(),DateUtil.DEFAULT_DATE_TIME));//截止时间
    ...
    public List<Dto> getTicketList(Scheme scheme, List<SchemeTicket> ticketList)
    {
        ...
        if(tkinfoList != null && tkinfoList.size() > 0)
        {
    ...
    public static void main(String[] args)
    {
        try
        {
            String lotteryCloseTime = "2016-01-27 00:00:00~2016-02-02 24:00:00";
            ...
            periodList = LotteryUtils.createPeriodsOfYear("1010",lotteryCloseTime,calendar);
            //periodList = createPeriodsByPeriodNum(lotteryCloseTime,"2016153",10);
