 ...
 * @author  mcdog
 */
public class Lottery1520Utils extends LotteryUtils
{
    /**
     * 生成期次(一年期次)
     * @author  mcdog
    ...
    public List<Map<String,String>> createPeriodsOfYear(String lotteryCloseTime, Calendar calendar)
    {
        ...
        //获取本年第一期次开奖的详细时间
        Calendar firstKjCalendar = getLotteryFirstKjDateOfYear(LotteryConstants.PL5,calendar);
        firstKjCalendar.setTime(DateUtil.parseDate(
                (DateUtil.formatDate(firstKjCalendar.getTime(),DateUtil.DEFAULT_DATE) + " " + LotteryConstants.lotteryEarliestKjTimeMaps.get(LotteryConstants.PL5)),
        ...
        for(int i = 0; i < dayOffset; i ++)
        {
            ...
            if(!isCloseTime)
            {
                ...
                periodMap.put("lotteryId",LotteryConstants.PL5);
                ...
                periodMap.put("sellEndTime",DateUtil.formatDate(getLotterySellEndTime(
                        LotteryConstants.PL5,firstKjCalendar).getTime(),DateUtil.DEFAULT_DATE_TIME));//截止时间
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            LotteryUtils lotteryUtils = new Lottery1520Utils();
            List<Map<String,String>> periodList = lotteryUtils.createPeriodsOfYear(lotteryCloseTime,calendar);
            //periodList = createPeriodsByPeriodNum(lotteryCloseTime,"2016001",10);
            ...
            /*String code = "12,35,6789,7,58:1:2";
            GamePluginAdapter plugin = (GamePluginAdapter) Thread.currentThread().getContextClassLoader().loadClass("com.caipiao.plugin.Lottery" + "1520").newInstance();//初始化彩种插件
            GameCastCode[] gameCastCodes = null;
            try
            {
                gameCastCodes = plugin.parseGameCastCodes(code);
                double money = 0d;//方案金额
                Map<String,String> wfMap = new HashMap<String,String>();//玩法集合
                for(GameCastCode castCode : gameCastCodes)
                {
                    money += castCode.getCastMoney();//单组投注串的总金额需要 x 倍数
                }
            ...
            scheme.setLotteryId("1520");
            scheme.setSchemeContent("12,35,6789,7,58:1:2");
            scheme.setDrawNumber("3,5,6,7,8");
