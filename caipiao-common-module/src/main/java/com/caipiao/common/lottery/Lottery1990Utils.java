 ...
 * @author  mcdog
 */
public class Lottery1990Utils extends LotteryUtils
{
    /**
     * 获取投注选项集合
     * @author  mcdog
    ...
    public List<Dto> getJcTzxxList(Scheme scheme,Map<String,Dto> matchMaps,Dto params)
    {
        ...
        for(String tzcode : tzcodes)
        {
            ...
            /*if(status == 1)
            {
                xx += "<font color='#FF0000'>" + ccinfoDto.getAsString("teamName");
                if(StringUtil.isNotEmpty(ccinfoDto.get("guestTeamName")))
                {
                    xx += " vs " + ccinfoDto.getAsString("guestTeamName");
                }
                xx += "</font>";
            }
            else
            {
                xx += ccinfoDto.getAsString("teamName");
                if(StringUtil.isNotEmpty(ccinfoDto.get("guestTeamName")))
                {
                    xx += " vs " + ccinfoDto.getAsString("guestTeamName");
                }
    ...
    public String getTicketTzxxs(Scheme scheme,Map<String,Dto> matchMaps,Dto ticketDto)
    {
        ...
        if(StringUtil.isNotEmpty(scheme.getSchemeContent()) && StringUtil.isNotEmpty(scheme.getSchemeSpContent()))
        {
            ...
            for(String tzcode : tzcodes)
            {
                ...
                if(StringUtil.isNotEmpty(ccinfoDto.get("guestTeamName")))
                {
                    tempxxsBuilder.append(" vs " + ccinfoDto.getAsString("guestTeamName"));
                }
