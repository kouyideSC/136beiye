 ...
 * @author  mcdog
 */
public class Lottery1030Utils extends LotteryUtils
{
    /**
     * 生成期次(一年期次)
     * @author  mcdog
    ...
    public List<Map<String,String>> createPeriodsOfYear(String lotteryCloseTime, Calendar calendar)
    {
        ...
        //获取本年第一期次开奖的详细时间
        Calendar firstKjCalendar = getLotteryFirstKjDateOfYear(LotteryConstants.FC3D,calendar);
        firstKjCalendar.setTime(DateUtil.parseDate(
                (DateUtil.formatDate(firstKjCalendar.getTime(),DateUtil.DEFAULT_DATE) + " " + LotteryConstants.lotteryEarliestKjTimeMaps.get(LotteryConstants.FC3D)),
        ...
        for(int i = 0; i < dayOffset; i ++)
        {
            ...
            if(!isCloseTime)
            {
                ...
                periodMap.put("lotteryId",LotteryConstants.FC3D);
                ...
                periodMap.put("sellEndTime",DateUtil.formatDate(getLotterySellEndTime(
                        LotteryConstants.FC3D,firstKjCalendar).getTime(),DateUtil.DEFAULT_DATE_TIME));//截止时间
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            scheme.setLotteryId("1030");
            scheme.setSchemeContent("1,4,7:2:3");
            scheme.setDrawNumber("1,1,7");
