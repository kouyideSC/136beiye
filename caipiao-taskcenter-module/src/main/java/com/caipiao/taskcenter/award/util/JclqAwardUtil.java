 ...
 */
public class JclqAwardUtil {
    private static Logger logger = LoggerFactory.getLogger(JclqAwardUtil.class);
     ...
     */
    public static boolean cancelJclqScheme(SchemeService schemeService, TicketService ticketService, JclqAwardInfo awardInfo) {
        try {
            List<Scheme> schemeList = schemeService.queryJcNoSuccessSchemeForEndTime(LotteryConstants.JCLQ, awardInfo.getMatchCode());
            ...
            for (Scheme scheme : schemeList) {
                ...
                logger.info("场次截止方案状态处理成功, schemeOrderId=" + scheme.getSchemeOrderId());
            }
            return true;
        } catch (Exception e) {
     ...
     * @return
     */
    public static boolean guoGuanJclqMatch(SchemeService schemeService, TicketService ticketService, JclqAwardInfo match, HashMap<String, GamePluginAdapter> pluginMap){
        try{
            ...
            //场次相关所有方案
            List<SchemeMatches> schemeMatchesList = schemeService.querySchemeForMatch(LotteryConstants.JCLQ, match.getMatchCode());
            if (StringUtil.isEmpty(schemeMatchesList)) {//该场无购彩方案
                LoggerUtil.printJcInfo("竞彩篮球-过关任务", match.getMatchCode(), "场次没有过关方案", logger);
                return true;
            }

            //当前场次关联的所有方案包含的场次
            Map<String, JclqAwardInfo> orderMatchs = schemeService.querySchemeForJclqMatch(LotteryConstants.JCLQ, match.getMatchCode());
            if(StringUtil.isEmpty(orderMatchs)) {
                LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "未找到当前场次相关的数据", logger);
                return false;
            }

            JclqAwardInfo currAwardInfo = orderMatchs.get(match.getMatchCode());
            if(StringUtil.isEmpty(currAwardInfo)) {
                LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "数据丢失", logger);
                return false;
            }
            if(currAwardInfo.getState() != LotteryConstants.MATCHJJ_STATE_FILE) {
                LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "未审核", logger);
                return false;
            }

            for (SchemeMatches schemeMatches : schemeMatchesList) {
                if(!schemeService.isAuditJclqSchemeForMatch(schemeMatches.getLotteryId(), schemeMatches.getSchemeId())) {
                ...
                if(StringUtil.isEmpty(tickets)){
                    LoggerUtil.printJcInfo("竞彩篮球-过关任务", match.getMatchCode(), "方案[" + schemeMatches.getSchemeOrderId() + "]没有待计奖的票", logger);
                ...
                for(SchemeTicket ticket : tickets) {
                    ...
                    if (plugin == null) {
                        LoggerUtil.printJcInfo("竞彩篮球-过关任务", match.getMatchCode(), "无法获取彩种插件" + ticket.getPlayTypeId(), logger);
                        break;
                    }
                    try {
                        ...
                        if (isWin >= 1) {
                            isWin = 2;
                            LoggerUtil.printJcInfo("竞彩篮球-过关任务", match.getMatchCode(), "方案号=" + ticket.getSchemeId() + " 票号=" + ticket.getTicketId() + "[过关]成功 中奖[" + outDel + "] 投注串=" + ticket.getCodes(), logger);
                        } else {
                            isWin = 1;
                            LoggerUtil.printJcInfo("竞彩篮球-过关任务", match.getMatchCode(), "方案号=" + ticket.getSchemeId() + " 票号=" + ticket.getTicketId() + "[过关]成功 未中奖[" + outDel + "] 投注串=" + ticket.getCodes(), logger);
                        }
                        ...
                        if (row == 0) {
                            LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "票号(" + ticket.getTicketId() + ") 更新数据库失败", null, logger);
                        }
                    } catch (Exception e) {
                        ...
                        if (ticket.getTicketStatus() < SchemeConstants.TICKET_STATUS_OUTED) {
                            ...
                            //打印异常日志
                            LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "票号(" + ticket.getTicketId() + ") 过关异常", e, logger);
                        }
                    }
                }
            }
            ...
            LoggerUtil.printJcError("竞彩篮球-过关任务", match.getMatchCode(), "过关异常", e, logger);
            return false;
        }
    }
     ...
     * @return
     */
    public static boolean prizeMoneySummary(SchemeService schemeService, TicketService ticketService, ActivityService activityService, UserService userService, MemCached memcached, JclqAwardInfo match) {
        try{
            if(StringUtil.isEmpty(match)){
                return false;
            }
            //需要进行金额汇总的票
            List<String> tickets = ticketService.queryTicketPrizeSummaryForMatch(LotteryConstants.JCLQ, match.getMatchCode());
            ...
            for(String schemeId : tickets) {
                ...
                for(SchemeTicket ticket : list) {//汇总金额及奖级[由于奖级不能在SQL中汇总，故使用程序汇总]
                    if(ticket.getBonusState() < 2 || ticket.getIsWin() == 0) {
                        success = false;//批次中有未过关的票
                        break;
                    }
                ...
                if(scheme == null) {
                    LoggerUtil.printJcError("竞彩篮球-奖金汇总任务", match.getMatchCode(), "找不到方案数据(方案号="+schemeId+")", null, logger);
                    continue;
                }
                //只有代办成功的方案才更新中奖状态
                    ...
                    if ((totalmoney + addmoney + addmoneysite) > 0) {//中奖
                        ...
                        if(scheme.getSchemeType().intValue() == SchemeConstants.SCHEME_TYPE_GD) {//跟单方案
                            ...
                            if(StringUtil.isEmpty(follow)) {
                                LoggerUtil.printJcError("竞彩篮球-跟单方案打赏任务", match.getMatchCode(), "找不到跟单关联表数据", logger);
                                continue;
                            }
                ...
                LoggerUtil.printJcInfo("竞彩篮球-奖金汇总任务", match.getMatchCode(), "方案号=" + schemeId + " 奖金汇总成功", logger);
            }
            tickets.clear();
            tickets = ticketService.queryTicketPrizeSummaryForMatch(LotteryConstants.JCLQ, match.getMatchCode());
            ...
            LoggerUtil.printJcError("竞彩篮球-奖金汇总任务", match.getMatchCode(), "奖金汇总异常", e, logger);
            return false;
        }
    }
    ...
    public static synchronized double addPrizeSummary(SchemeService schemeService, ActivityService activityService, UserService userService, Scheme scheme, String matchCode, MemCached memCached) {
        try{
            ...
            List<SchemeMatches> matchesList = schemeService.querySchemeInfoByMatches(LotteryConstants.JCLQ, scheme.getId());
            ...
            String acKey = "addbonus_ac_" + LotteryConstants.JCLQ;
            if(memCached.contains(acKey)) {
                activityAddBonusList = (List<ActivityAddBonus>) memCached.get(acKey);
            } else {
                activityAddBonusList = activityService.queryLotteryAddActivityList(LotteryConstants.JCLQ);
            ...
            for(ActivityAddBonus activityAddBonus : activityAddBonusList) {
                ...
                if (StringUtil.isEmpty(activityAddBonus.getAddBonusRate())) {
                    LoggerUtil.printJcError("竞彩篮球-计算加奖任务", "未配置正确加奖比例[加奖活动名称=" + activityAddBonus.getActivityName() + "]", logger);
                    continue;//未正确配置加奖比例
                }
                //查询参与活动的所有用户
                ...
                if (contents.length != 3) {
                    LoggerUtil.printJcError("竞彩篮球-计算加奖任务", "投注串格式错误[方案号=" + scheme.getSchemeOrderId() + "]", logger);
                    continue;
                }
                ...
                if (activityAddBonus.getBalance() >= activityAddBonus.getMaxMoney()) {
                    LoggerUtil.printJcError("竞彩篮球-计算加奖任务", "活动剩余额度不足[加奖活动名称=" + activityAddBonus.getActivityName() + " 方案编号=" + scheme.getSchemeOrderId() + "]", logger);
                    continue;//可用额度不足
                }
                //查询用户当前期已经加奖金额
                ...
                if (userDayAddPrizeSum >= activityAddBonus.getUserDayLimit()) {
                    LoggerUtil.printJcInfo("竞彩篮球-计算加奖任务", "方案号=" + scheme.getSchemeOrderId() + "不享受加奖(单日加奖[金额" + userDayAddPrizeSum + "元]已达到上限)", logger);
                    continue;
                }
                //计算加奖奖金
                ...
                if (addPrize > 0) {
                    ...
                    if (row > 0) {//流水添加成功
                        ...
                        LoggerUtil.printJcInfo("竞彩篮球-计算加奖任务", matchCode, "方案号=" + scheme.getSchemeOrderId() + ",中奖金额=" + scheme.getPrizeTax() + ",加奖活动名称=" + activityAddBonus.getActivityName() + ",加奖比例=" + rate + ",加奖金额=" + addPrize + " 计算加奖金额成功", logger);
                    }
            ...
            LoggerUtil.printJcError("竞彩篮球-计算加奖任务", matchCode, "方案号="+scheme.getSchemeOrderId()+" 计算加奖异常", e, logger);
            return 0;
        }
    }
     ...
     * @return
     */
    public static boolean followSchemeRewardMoney(SchemeService schemeService, JclqAwardInfo match) {
        try{
            if(StringUtil.isEmpty(match)){
                return false;
            }
            //查询中奖的神单发单人收获打赏金额
            List<SchemeFollow> followList = schemeService.querySchemeFollowListByRewards(LotteryConstants.JCLQ, match.getMatchCode());
            ...
            for(SchemeFollow follow : followList) {
                ...
                LoggerUtil.printJcInfo("竞彩篮球-汇总神单打赏任务", match.getMatchCode(), "方案号=" + follow.getSenderSchemeId() + " 神单打赏金汇总成功", logger);
            }
            followList.clear();
            return true;
        } catch (Exception e){
            LoggerUtil.printJcError("竞彩篮球-汇总神单打赏任务", match.getMatchCode(), "神单打赏金汇总异常", e, logger);
            return false;
        }
    }
     ...
     * @return
     */
    public static boolean authSendSmallMoney(SchemeService schemeService, UserService userService, JclqAwardInfo match){
        try{
            if(StringUtil.isEmpty(match)){
                return false;
            }
            ...
            //满足自动派奖要求的方案
            List<Scheme> schemeList = schemeService.queryAutoSendMoneySchemeList(LotteryConstants.JCLQ, maxPrize);
            if(StringUtil.isEmpty(schemeList)){
                return true;
            }
            ...
            for(Scheme scheme : schemeList) {
                ...
                if(success == 1){
                    LoggerUtil.printJcInfo("竞彩篮球-自动派奖任务", "方案号=" + scheme.getSchemeOrderId() + " (奖金小于"+maxPrize+")自动派奖成功【派送奖金:"+scheme.getPrizeTax()+",税前奖金:"+scheme.getPrize()+"】", logger);
                } else {
                    LoggerUtil.printJcInfo("竞彩篮球-自动派奖任务", "方案号=" + scheme.getSchemeOrderId() + " (奖金小于"+maxPrize+")自动派奖失败【税后奖金:"+scheme.getPrizeTax()+"】", logger);
                }
            }
            schemeList.clear();
            schemeList = schemeService.queryAutoSendMoneySchemeList(LotteryConstants.JCLQ, maxPrize);
            ...
            LoggerUtil.printJcError("竞彩篮球-自动派奖任务", match.getMatchCode(), "自动派奖异常", e, logger);
            return false;
        }
    }
     ...
     * @return
     */
    public static boolean followSchemeUserDataStatis(SchemeService schemeService, JclqAwardInfo match){
        try{
            if(StringUtil.isEmpty(match)){
                return false;
            }
            //查询某场比赛相关已结算的神单用户编号
            List<Long> userList = schemeService.queryFollowUserIdForMatch(LotteryConstants.JCLQ, match.getMatchCode());
            ...
            for(Long userId : userList) {
                ...
                //查询最近一周用户神单数据
                int hitSum = 0;
                List<Dto> schemeWeekList = schemeService.queryUserFollowForWeek(LotteryConstants.JCLQ, userId);
                if(StringUtil.isNotEmpty(schemeWeekList)) {
                    ...
                    followStatis.put("weekHitRate", CalculationUtils.bankerAlgoNum((double)hitSum/schemeWeekList.size())*100);
                    followStatis.put("weekBuyMoney", money);
                    followStatis.put("weekHitMoney", prize);
                    followStatis.put("weekWinRate", CalculationUtils.bankerAlgoNum(prize/money)*100);
                ...
                //***************************近一月数据统计********************************
                //查询最近一月用户神单数据
                Dto schemeMonth = schemeService.queryUserFollowStatisForMonth(LotteryConstants.JCLQ, userId);
                if(StringUtil.isNotEmpty(schemeMonth)) {
                    ...
                    followStatis.put("monthHitRate", CalculationUtils.bankerAlgoNum((double)mHitSum/mSum)*100);
                    followStatis.put("monthBuyMoney", mMoney);
                    followStatis.put("monthHitMoney", mPrize);
                    followStatis.put("monthWinRate", CalculationUtils.bankerAlgoNum(mPrize/mMoney)*100);
                ...
                int hitCount = 0;
                List<Long> tenRedList = schemeService.queryUserNearTenFollowScheme(LotteryConstants.JCLQ, userId);
                ...
                followStatis.put("lotteryId", LotteryConstants.JCLQ);
                //最近一周有分享神单 才统计收到打赏总金额
                if(schemeWeekList.size() > 0) {
                    Dto statis = schemeService.queryUserFollowRewardMoneyStatis(LotteryConstants.JCLQ, userId);
                    ...
                    followStatis.put("hitRate", CalculationUtils.bankerAlgoNum((double)hit/sum)*100);
                    followStatis.put("buyMoney", money);
                    followStatis.put("hitMoney", prize);
                    followStatis.put("winRate", CalculationUtils.bankerAlgoNum((double)prize/money)*100);
                ...
                LoggerUtil.printJcInfo("竞彩篮球-用户神单数据任务", "用户编号=" + userId + " 数据处理成功", logger);
            }
            userList.clear();
            return true;
        } catch (Exception e){
            LoggerUtil.printJcError("竞彩篮球-用户神单数据任务", match.getMatchCode(), "用户神单数据处理异常", e, logger);
            return false;
        }
    }
     ...
     * @throws Exception
     */
    private static boolean processJcGuoGuanCodes(SchemeTicket ticket, Map<String, JclqAwardInfo> orderMatchs, Map<String, String> detail, double[] results, GamePluginAdapter plugin) throws Exception{
        ...
        for (int index = 0; index < tcodes.length; index++) {
            ...
            for (int jt = 0; jt < lst.size(); jt++) {
                LqCastCode ccode = (LqCastCode) lst.get(jt);
                ...
                for(int t = 0; t < bcs.length; t++){
                    ...
                    for (int j = 0; j <_lst.size(); j++) {
                        LqCastCode _bccode = (LqCastCode) _lst.get(j);
                        int num = 0;
                        double total = 0, tax = 0;
                        List<BingoUtil> bingoList = new ArrayList<BingoUtil>();
                        List<LqItemBean> lstItem = _bccode.getLqItemList();
                        for (int k = 0; k < lstItem.size(); k++) {
                            LqItemBean item = lstItem.get(k);
                            String itemId = item.getItemid();
                            JclqAwardInfo award = orderMatchs.get(itemId);
                            if (award == null) {
                            ...
                            int rel = Long.bitCount(item.getCode() & award.getBingoCode(item, mapTicketSp));
                            bingo.setTime(rel);
                            if (rel > 1) {
                            ...
                            if (comList != null) {//结果和串关数得出是否中奖
                                for (int k = 0; k < comList.size(); k++) {
                                    ...
                                    if (d1 > 0) {
                                        ...
                                        logger.info("竞彩篮球-过关中... 方案号=" + ticket.getSchemeId() + " 票号=" + ticket.getTicketId() + " 串关方式=" + getPassType(n) + " 单注奖金=" + dt + " 税后奖金=" + da);
                        ...
                        logger.info("竞彩篮球-过关中... 方案号=" + ticket.getSchemeId() + " 票号=" + ticket.getTicketId() + " 投注倍数=" + ticket.getMultiple() + " 中奖注数=" + num + " 税前奖金=" + total + " 税后奖金=" + tax);
    ...
    private static void addInfo(Map<String, String> winInfo, String curInfo) {
        ...
        String[] winfos = com.mina.rbc.util.StringUtil.splitter(curInfo, ";");
        for(String wf : winfos) {
            String[] fo = com.mina.rbc.util.StringUtil.splitter(wf, "|");
            if(fo.length != 4) {
                continue;
            }
            String chuan = fo[0];
    ...
    public static void info(SchemeTicket ticket, String matchID, String info, Logger logger) {
        logger.info("竞彩篮球-过关中... 方案号=" + ticket.getSchemeId() + ",票号=" + ticket.getTicketId() + " 场次=" + matchID + " " + info);
    }
    ...
    public static void error(SchemeTicket ticket, String matchID, String info, Logger logger) {
        logger.error("竞彩篮球-过关中... 方案号=" + ticket.getSchemeId() + ",票号=" + ticket.getTicketId() + " 场次=" + matchID + " " + info);
    }
