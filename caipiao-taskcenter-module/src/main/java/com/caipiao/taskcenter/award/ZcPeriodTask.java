...
/**
 * 老足彩期次任务
 * Created by kouyi on 2017/12/30.
 */
@Component("zcPeriodTask")
public class ZcPeriodTask extends PeriodAwardUtil {
    private static Logger logger = LoggerFactory.getLogger(ZcPeriodTask.class);
    ...
    static {
        lottery.add(LotteryConstants.SFC);//胜负彩
        lottery.add(LotteryConstants.RXJ);//任九
    }
     ...
     * 期次状态处理
     */
    public void zcPeriodProcessTask() {
        try {
            // 3分钟增量加载新期次
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    ...
                    if (state == 99) {
                        LoggerUtil.printInfo("老足彩-期次任务", period, "卸载完成", logger);
                        endPeriod.add(key);
                    }
                } catch (Exception e) {
                    LoggerUtil.printError("老足彩-期次任务", period, "处理异常", e, logger);
                }
            }
        ...
        switch (state) {
            case LotteryConstants.STATE_DEFAULT : {//期次截止:投注截止
                ...
                if (endTime.getTime() < System.currentTimeMillis()) {
                    ...
                    LoggerUtil.printInfo("老足彩-期次截止任务", period, "截止销售", logger);
                    //当前期次文件任务
                    taskService.saveTask(new Task(Constants.periodUpdateTaskMaps.get(period.getLotteryId())));
                }
                break;
            }
            case LotteryConstants.STATE_ONE: {//方案撤单:老足彩期次官方截止前1分钟还没出票
                ...
                if (endTime.getTime() < (System.currentTimeMillis() + 60000)) {
                    ...
                    if(flag) {
                        ...
                        periodService.updatePeriodStatusById(period);
                        LoggerUtil.printInfo("老足彩-自动撤单任务", period, "撤单完成", logger);
                    }
                }
                break;
            }
            case LotteryConstants.STATE_TWO: {//抓取开奖号码:老足彩官网开奖时间后1分钟抓取
                Date awardTime = period.getDrawNumberTime();
                if (awardTime.getTime() < (System.currentTimeMillis() - 60000)) {
                    ...
                    if(StringUtil.isNotEmpty(period.getDrawNumber()) && period.getGrabSuccess() && PrizesUtil.isNumber(period.getDrawNumber(), period.getLotteryId())) {
                        ...
                        periodService.updatePeriodStatusById(period);
                        LoggerUtil.printInfo("老足彩-抓取开奖号任务", period, "抓取开奖号完成 开奖号=" + period.getDrawNumber(), logger);
                    }
                }
                break;
            }
            case LotteryConstants.STATE_THREE: {//老足彩开奖号码运营审核
                //LoggerUtil.printInfo("老足彩-运营审核开奖号任务", period, "待审核... 开奖号=" + period.getDrawNumber(), logger);
                break;
            }
            case LotteryConstants.STATE_FOUR: {//开奖号同步到用户数字彩订单
                if(isCancel(period)) {
                    LoggerUtil.printError("老足彩-系统审核开奖号任务", period, "运营审核开奖号不正确 回退状态重新审核 开奖号=" + period.getDrawNumber(), logger);
                    period.setState(LotteryConstants.STATE_THREE);
                    periodService.updatePeriodStatusById(period);
                    break;
                }
                ...
                if(flag) {
                    ...
                    periodService.updatePeriodStatusById(period);
                    LoggerUtil.printInfo("老足彩-开奖号同步订单任务", period, "开奖号同步完成 开奖号=" + period.getDrawNumber(), logger);
                }
                break;
            }
            case LotteryConstants.STATE_FILE: {//中奖匹配:计奖票表每张票中奖注数和奖级
                ...
                if(plugin == null) {
                    LoggerUtil.printInfo("老足彩-中奖匹配任务", period, "无法获取彩种插件" + period.getLotteryId(), logger);
                    break;
                }
                ...
                if(flag) {//全部完成
                    ...
                    periodService.updatePeriodStatusById(period);
                    LoggerUtil.printInfo("老足彩-中奖匹配任务", period, "中奖匹配完成", logger);
                }
                break;
            }
            case LotteryConstants.STATE_SIX: {//计算奖金:计算票表每张票的中奖奖金
                ...
                if(flag) {//全部完成
                    ...
                    periodService.updatePeriodStatusById(period);
                    LoggerUtil.printInfo("老足彩-计算奖金任务", period, "计算奖金完成", logger);
                }
                break;
            }
            case LotteryConstants.STATE_SEVEN: {//奖金汇总:票表奖金汇总到订单表中的用户订单
                ...
                if(flag) {//全部完成
                    ...
                    periodService.updatePeriodStatusById(period);
                    LoggerUtil.printInfo("老足彩-奖金汇总任务", period, "奖金汇总完成", logger);
                }
                break;
            }
            case LotteryConstants.STATE_EIGHT: {//核对奖金:核对奖金是否正确
                ...
                periodService.updatePeriodStatusById(period);
                LoggerUtil.printInfo("老足彩-奖金核对任务", period, "奖金核对完成", logger);
                break;
            }
            case LotteryConstants.STATE_NINE: {//自动派奖:系统自动派奖
                ...
                if(flag) {//全部完成
                    ...
                    periodService.updatePeriodStatusById(period);
                    LoggerUtil.printInfo("老足彩-自动派奖任务", period, "自动派奖完成", logger);
                }
                break;
            }
            case LotteryConstants.STATE_TEN: {//过关统计-老足彩暂不做任何逻辑
                period.setState(LotteryConstants.STATE_ELEVEN);
                periodService.updatePeriodStatusById(period);
                LoggerUtil.printInfo("老足彩-过关统计任务", period, "过关统计完成", logger);
                break;
            }
            case LotteryConstants.STATE_ELEVEN: {//用户战绩统计-老足彩暂不做任何逻辑
                period.setState(LotteryConstants.STATE_TWELVE);
                periodService.updatePeriodStatusById(period);
                LoggerUtil.printInfo("老足彩-战绩统计任务", period, "战绩统计完成", logger);
                break;
            }
            case LotteryConstants.STATE_TWELVE: {//派送返点
                period.setState(LotteryConstants.STATE_THIRTEEN);
                periodService.updatePeriodStatusById(period);
                LoggerUtil.printInfo("老足彩-派送返点任务", period, "派送返点完成", logger);
                break;
            }
            case LotteryConstants.STATE_THIRTEEN: {//期次处理结束
                period.setState(LotteryConstants.STATE_END);
                periodService.updatePeriodStatusById(period);
                LoggerUtil.printInfo("老足彩-任务结束", period, "期次处理结束", logger);
                break;
            }
