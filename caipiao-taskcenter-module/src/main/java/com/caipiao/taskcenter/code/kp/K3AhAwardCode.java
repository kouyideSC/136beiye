 ...
 */
public class K3AhAwardCode extends ResultDataUtil implements GrabDrawCode {
    private static Logger logger = LoggerFactory.getLogger(K3AhAwardCode.class);
    ...
    public void GrabGuanWang(Period period) {
        try {
            ...
            Document doc = cUrl(K3AH_CJW_PERIOD_URL, HOST_CJW);
            Elements tables = doc.getElementsByClass("kjjg_table");
            if(StringUtil.isEmpty(tables)) {
                logger.info("[彩经网-安徽快3期次("+period.getPeriod()+")开奖号码抓取] 获取kjjg_table标签无数据");
                return;
            }
            ...
            logger.error("[彩经网-安徽快3期次("+period.getPeriod()+")开奖号码抓取异常,原因=" + ex.getMessage());
        }
    }
    ...
    public static void main(String[] args) {
        ...
        new K3AhAwardCode().GrabGuanWang(p);
