...
<%
    ...
    request.setAttribute("sidebar_mcode","menu_xiaoshou_rebate");
%>
<html lang="en" class="app">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="EmulateIE9" >
    <title>销售返点设置-列表</title>
                ...
                <div class="word-list-show pull-left">
                    ...
                    <div class="list-box">
                        <ul class="list-box-show">
                            ...
                            <li scolumn="tgUserNumber">推广用户</li>
                            <li scolumn="sumMoney">代理总销量</li>
                            <li scolumn="registerTime">注册时间</li>
                            <li scolumn="bankIsBind">银行绑定</li>
                            <li scolumn="qq">邀请码</li>
...
$(function ()
{
    ...
    $(document).on('click','.set_user_detail',function()
    {
        ...
        $('#detailCard').fillWithUrl('${pageContext.request.contextPath}/sale/rebate/detail?id=' + $trNode.attr('editid'));
    });
    ...
    {
        var $trNode = $(this).parents('tr').first();
        $('#confirmModal').find('.modal-body p').first().html('确定将销售员('+ $trNode.attr('nname') +')贬为庶民吗？');
    ...
    $('#confirmModalSureBtn').on('click',function()
    {
        ...
        debugger;
        $.ajax({
            url : '${pageContext.request.contextPath}/sale/rebate/cancelsale?id=' + uid,
            type : 'get',
            dataType : 'json',
...
var querydatas = function (pnum)
{
    ...
    data.url = '${pageContext.request.contextPath}/sale/rebate/list';
    data.page = pnum;
    ...
    pageAjax(data, function ()
    {
        ...
        if(data.datas.list && data.datas.list.length > 0)
        {
            ...
            {
                html += '<tr editid="' + n.id + '" nname="' + n.nickName + '">';
                html += '<td>';
                html += '<div class="btn-group clearfix">';
                html += '<a opauthority="btn_xiaoshou_rebate_qxxszg" href="javascript:;" class="label label-info mright10 edit_vote">贬为庶民</a>';
                html += '</div>';
                html += '</td>';
                html += '<td scolumn="nickName"><a dtauthority="menu_xiaoshou_rebate" href="javascript:;" class="set_user_detail">' + n.nickName + '</a></td>';
                html += '<td scolumn="realName">' + (n.realName!=''?n.realName:'--') + '</td>';
                ...
                html += '<td scolumn="tgUserNumber">' + n.tgUserNumber + '人</td>';
                html += '<td scolumn="sumMoney">' + (n.zjSaleMoney+n.zjSaleZhuiHaoMoney+n.dlSaleMoney+n.dlZhuiHaoSaleMoney+n.dlUserSaleMoney+n.dlUserZhuiHaoSaleMoney+n.yhSaleMoney+n.yhZhuiHaoSaleMoney) + '元</td>';
                html += '<td scolumn="registerTime">' + n.registerTime + '</td>';
                html += '<td scolumn="bankIsBind">' + (n.bankIsBind==0?'未绑定':'已绑定') + '</a></td>';
                html += '<td scolumn="qq">' + n.qq + '</a></td>';
