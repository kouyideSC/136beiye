...
$(function ()
{
    ...
    $(document).on('click','.set_user_detail',function()
    {
        ...
        $('#detailCard').fillWithUrl('${pageContext.request.contextPath}/sale/proxy/detail?id=' + $trNode.attr('editid'));
    });
    ...
    {
        var $trNode = $(this).parents('tr').first();
        $('#confirmModal').find('.modal-body p').first().html('确定取消用户('+ $trNode.attr('nname') +')代理权限吗？');
    ...
    $('#confirmModalSureBtn').on('click',function()
    {
        ...
        debugger;
        $.ajax({
            url : '${pageContext.request.contextPath}/sale/lower/cancelproxy?id=' + uid,
            type : 'get',
            dataType : 'json',
...
var querydatas = function (pnum)
{
    ...
    data.url = '${pageContext.request.contextPath}/sale/proxy/list';
    data.page = pnum;
    ...
    pageAjax(data, function ()
    {
        ...
        if(data.datas.list && data.datas.list.length > 0)
        {
            $.each(data.datas.list, function (i, n)
            {
                ...
                html += '<a href="javascript:;" class="label label-info mright10 edit_vote">取消代理</a>';
                html += '</div>';
                html += '</td>';
                ...
                html += '<td scolumn="sumMoney">' + (n.dlSaleMoney+n.dlSaleZhuiHaoMoney+n.dlUserSaleMoney+n.dlUserZhuiHaoSaleMoney) + '元</td>';
                html += '<td scolumn="registerTime">' + n.registerTime + '</td>';
                html += '<td scolumn="bankIsBind">' + (n.bankIsBind==0?'未绑定':'已绑定') + '</a></td>';
                html += '<td scolumn="qq">' + n.qq + '</td>';
