...
$(function ()
{
    ...
    {
        $.ajax({
            url : '${pageContext.request.contextPath}/user/black/cancelBlack',
            type : 'post',
            dataType : 'json',
            data : operatorData,
...
{
    $('#confirmModal').find('.modal-body p').first().html('确认取消用户黑名单吗？');
...
var querydatas = function (pnum)
{
    ...
    data.url = '${pageContext.request.contextPath}/user/black/list';
    data.page = pnum;
    ...
    pageAjax(data, function ()
    {
        ...
        if(data.datas.list && data.datas.list.length > 0)
        {
            ...
            {
                html += '<tr editid="' + n.id + '">';
                html += '<td><div class="btn-group clearfix"><a dtauthority="btn_weihu_lottery_edit" class="label label-failed" href="javascript:;" onclick=\"editUserStatus(\'' + n.id + '\')\">取消黑名单</a></div></td>';
                ...
                html += '<td scolumn="saleMoney">' + n.consume + '元</td>';
