            ...
            <div class="core-table pull-left" style="position: relative; width:100%">
                ...
                <table id="dataTables" style="min-width:1200px;" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        ...
                        <th scolumn="differPrize">出票商兑奖奖金与网站计奖奖金有差异金额</th>
                        <th scolumn="differNumber">出票商兑奖奖金与网站计奖奖金有差异笔数</th>
                        <th scolumn="votePrize">出票商兑奖金额</th>
                        <th scolumn="sitePrize">网站计奖金额</th>
                ...
                <div class="word-list-show pull-left">
                    ...
                    <div class="list-box">
                        <ul class="list-box-show">
                            ...
                            <li scolumn="differPrize">出票商兑奖奖金与网站计奖奖金有差异金额</li>
                            <li scolumn="differNumber">出票商兑奖奖金与网站计奖奖金有差异笔数</li>
                            <li scolumn="votePrize">出票商兑奖金额</li>
                            <li scolumn="sitePrize">网站计奖金额</li>
...
$(function ()
{
    ...
    $(document).on('click','#export',function()
    {
        ...
        window.location.href = '${pageContext.request.contextPath}/check/prize/export?' + params.substring(1);
    });
    querydatas(1);
});
//获取列表数据
var querydatas = function (pnum)
{
    ...
    var data = $('.operate').getConditionValue();
    data.url = '${pageContext.request.contextPath}/check/prize/list';
    data.page = pnum;
    ...
    pageAjax(data, function ()
    {
        ...
        if(data.datas.list && data.datas.list.length > 0)
        {
            $.each(data.datas.list, function (i, n)
            {
                ...
                html += '<td scolumn="differPrize">' + n.differPrize + '元</td>';
                html += '<td scolumn="differNumber">' + n.differNumber + '笔</td>';
                html += '<td scolumn="votePrize">' + n.votePrize + '元</td>';
                html += '<td scolumn="sitePrize">' + n.sitePrize + '元</td>';
